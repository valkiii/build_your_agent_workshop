...
- Coders: pick an extension to build — add a new RSS source, tune the tag-generation prompt, or attempt the quiz feature if not already wired in. Offer 2–3 concrete "stretch goal" options so nobody stalls on a blank page. (A worked example of a second "Act" output already lives in the repo: `narrator_agent.py` turns the kept articles into a spoken two-host podcast — good to point at for "the agent can act in more than one way". Because every run saves a checkpoint, you can curate once and demo the podcast as a separate step: `python src/main.py --from "output/Curated News of <date>.json" --audio`, or the app's **Step 2**.)
