...
- Coders: pick an extension to build — add a new RSS source, tune the tag-generation prompt, or attempt the quiz feature if not already wired in. Offer 2–3 concrete "stretch goal" options so nobody stalls on a blank page. (A worked example of a second "Act" output already lives in the repo: `narrator_agent.py` turns the approved articles into a spoken two-host podcast — good to point at for "the agent can act in more than one way".)
